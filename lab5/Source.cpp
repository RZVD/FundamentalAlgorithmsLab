...
/**
 *  As the fill factor grows there are more colissions and
 *  therefore the effort needed to find elements gets higher
 */
...
void demo() {
    ...
    Entry* test[] = {
        new Entry{5, random_string()},
        new Entry{5, random_string()},
        new Entry{10, random_string()}, 
        new Entry{9, random_string()},
    ...
    hash_delete(arr, test[0]);
    hash_delete(arr, test[2]);
    ...
    puts("");
    for (int i = 0; i < 5; i++) {
        print_entry(hash_search_(arr, test[i], _c));
    }

    puts("-----------------------");
    print_entry(hash_search_(arr, NULL, _c));
}
...
void perf_del() {
    ...
    while (lim1 > lim2) {
        hash_delete(arr, search.back()-> id);
        search.pop_back();
        lim1--;
    }
