...
// order:                       black, blue, green, cyan, red, magenta, yellow,
// white, gray, lblue, lgreen, lcyan, lred, lmagenta, lyellow, bwhite
const unsigned char CONS_COLORS_FG[] = {30, 34, 32, 36, 31, 35, 33, 37,
                                        90, 94, 92, 94, 91, 95, 93, 97};
const unsigned char CONS_COLORS_BG[] = {49,  44,  42,  46,  41,  45,  43,  47,
    ...
    do {
        ...
        if (strcmp(token, "exit") == 0 || strcmp(token, "quit") == 0 ||
            ...
            validCommand = 1;
                   ...
                   strcmp(token, "performance") == 0) {
            c->command = COMMAND_PERF;
            validCommand = 1;
        } else if (strcmp(token, "neighb") == 0 ||
                   strcmp(token, "neighbors") == 0 ||
                   strcmp(token, "bfs") == 0 ||
                   strcmp(token, "bfs_step") == 0 ||
                   strcmp(token, "bfs_tree") == 0 ||
                   strcmp(token, "path") == 0) {
            if (strcmp(token, "neighb") == 0 ||
                strcmp(token, "neighbors") == 0) {
            ...
            if (c->p1.row >= 0 && c->p1.col >= 0 && c->p1.row < grid->rows &&
                c->p1.col < grid->cols) {
                if (c->command == COMMAND_PATH) {
                    ...
                    if (c->p2.row >= 0 && c->p2.col >= 0 &&
                        c->p2.row < grid->rows && c->p2.col < grid->cols) {
                        validCommand = 1;
                    }
                } else {
                    validCommand = 1;
    ...
    for (;;) {
        ...
        if (cmd.command == COMMAND_EXIT) {
            ...
            if (grid.mat[cmd.p1.row][cmd.p1.col] & MASK_WALL) {
                snprintf(err, 100,
                         "Starting position (%d, %d) is inside a wall.",
                         cmd.p1.row, cmd.p1.col);
            ...
            for (int i = 0; i < nrNeighb; ++i) {
                if (neighb[i].row >= 0 && neighb[i].row < grid.rows &&
                    ...
                    if (grid.mat[neighb[i].row][neighb[i].col] & MASK_WALL) {
                        snprintf(err, 100,
                                 "Position (%d, %d) is inside a wall.",
                                 neighb[i].row, neighb[i].col);
                        errors.push_back(err);
                    }
                } else {
                    snprintf(err, 100, "Position (%d, %d) is outside the grid.",
                             neighb[i].row, neighb[i].col);
                    errors.push_back(err);
                }
            }
        } else if (cmd.command == COMMAND_BFS ||
                   cmd.command == COMMAND_BFS_STEP ||
                   cmd.command == COMMAND_BFS_TREE) {
            grid2 = grid;
            ...
            for (i = 0; i < graph.nrNodes; ++i) {
                if (graph.v[i]->position.row == cmd.p1.row &&
                    graph.v[i]->position.col == cmd.p1.col) {
                    s = graph.v[i];
                    break;
                }
            }
            if (s == NULL) {
                snprintf(err, 100, "Invalid starting position (%d, %d).",
                         cmd.p1.row, cmd.p1.col);
                errors.push_back(err);
            } else {
                bfs(&graph, s);
            ...
            for (i = 0; i < graph.nrNodes; ++i) {
                ...
                if (graph.v[i]->position.row == cmd.p2.row &&
                    graph.v[i]->position.col == cmd.p2.col) {
                    end = graph.v[i];
                }
            ...
            if (start == NULL || end == NULL || start == end) {
                snprintf(err, 100,
                         "Invalid starting and/or end position (%d, %d) -> "
                         "(%d, %d).",
